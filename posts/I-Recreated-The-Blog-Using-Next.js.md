...
그래서 생각했죠. 새로 만들고 다 갈아엎어버려야겠다고요. 그래서 어떻게 만들까 하다가 제가 좋아하는 `React`로 만들려고 했는데 제가 존경하는 어떤 분께서 `Next.JS`가 쉽고 좋다고 추천을 해줘서 `Next.JS`를 사용하기로 했고 `Next.JS`에 정적 사이트를 쉽게 만들 수 있도록 도와주는 `Next-Contentlayer`라는 모듈이 있길래 그거 끌고 와서 만들었어요.
